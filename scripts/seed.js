...
async function seed() {
  ...
  try {
    await supabase.from('zebra_tax_records').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  } catch (e) {
    console.warn('Warning: Could not clean old zebra_tax_records. Continuing...');
  }
  try {
    await supabase.from('zebra_payroll_audits').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  } catch (e) {
    console.warn('Warning: Could not clean old zebra_payroll_audits. Continuing...');
  }
    ...
    .insert([
      {
        ...
        kyc_root: root,
        encrypted_view_key: '04a3b8d4f6c9e0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef012',
  ...
  try {
    const { data: taxData, error: taxError } = await supabase
      ...
      .insert([
        {
          payroll_hash: txHash,
          tax_authority: 'GDTAXAUTHORITYXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX',
          tax_amount: 3500.00,
          net_amount: 20000.00,
          status: 'complete'
        }
    ...
    if (taxError) {
      console.warn('Warning: Could not seed zebra_tax_records (possibly table needs to be created):', taxError.message);
    } else {
      console.log('Seeded zebra_tax_records:', taxData);
    }
  } catch (err) {
    console.warn('Warning: Error inserting into zebra_tax_records:', err.message);
  }
