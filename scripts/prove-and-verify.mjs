...
const ID = process.env.ZEBRA_CONTRACT_ID || "CCLTVNPYS5H2AY4OTYIYDU57XYO4S5OZQE435ZZX2TFUVYDAIS6B53N5";
...
run("bb", ["prove", "--scheme", "ultra_honk", "--oracle_hash", "keccak",
  "--bytecode_path", `${TARGET}/payroll_circuit.json`, "--witness_path", `${TARGET}/payroll_circuit.gz`,
  "--output_path", TARGET, "--output_format", "bytes_and_fields"]);
run("bb", ["write_vk", "--scheme", "ultra_honk", "--oracle_hash", "keccak",
  "--bytecode_path", `${TARGET}/payroll_circuit.json`, "--output_path", TARGET, "--output_format", "bytes_and_fields"]);
...
const out = run("stellar", ["contract", "invoke", "--id", ID, "--source", "deployer",
  "--network", "testnet", "--send=yes", "--", "verify_proof",
