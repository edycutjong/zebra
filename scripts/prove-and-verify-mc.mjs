...
const ID = process.env.ZEBRA_MC_CONTRACT_ID || "CBXJ75PDFAMMGL2VHEXBJX2UT3GSWPHEZZULOFZWUZLN57V6UUBOL6NB";
...
run("bb", ["prove", "--scheme", "ultra_honk", "--oracle_hash", "keccak",
  "--bytecode_path", `${TARGET}/payroll_mc.json`, "--witness_path", `${TARGET}/payroll_mc.gz`,
  "--output_path", TARGET, "--output_format", "bytes_and_fields"]);
run("bb", ["write_vk", "--scheme", "ultra_honk", "--oracle_hash", "keccak",
  "--bytecode_path", `${TARGET}/payroll_mc.json`, "--output_path", TARGET, "--output_format", "bytes_and_fields"]);
...
const out = run("stellar", ["contract", "invoke", "--id", ID, "--source", "deployer",
  "--network", "testnet", "--", "verify_mc_proof",
